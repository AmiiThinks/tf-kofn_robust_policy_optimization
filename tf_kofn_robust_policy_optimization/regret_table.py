...
class RegretTable(object):
    ...
    def new_ir_table(horizon, num_states, num_actions, **kwargs):
        ...
        if 'name' in kwargs:
            ...
            name = 'ir_table'
        return tf.Variable(init, name=name, trainable=False, **kwargs)
    ...
    def indices_at_timestep_ir(sequences, t, num_states):
        return list(
            product(range(t * num_states, (t + 1) * num_states),
                    range(sequences.shape[1])))
    ...
    def indices_at_timestep_pr(sequences, t, num_states, num_actions):
        ...
        return list(product(range(n, next_n), range(sequences.shape[1])))
    ...
    def update_regrets_at_timestep_ir(regrets, t, num_states, inst_regrets,
        ...
        return tf.compat.v1.scatter_nd_update(regrets, indices, flat_updates,
                                              **kwargs)
    ...
    def update_regrets_at_timestep_pr(regrets, t, num_states, num_actions,
        ...
        indices = RegretTable.indices_at_timestep_pr(regrets, t, num_states,
                                                     num_actions)
        flat_updates = tf.reshape((RegretTable.sequences_at_timestep_pr(
            regrets, t, num_states, num_actions) + inst_regrets),
                                  shape=[tf.size(inst_regrets)])
    ...
    def strat(self):
        return l1_projection_to_simplex(self.regrets,
                                        axis=tf.rank(self.regrets) - 1)
...
class PrRegretTable(RegretTable):
    ...
    def instantaneous_regrets(self, weighted_sequence_rewards):
        ...
        for t in range(self.horizon() - 1, -1, -1):
            ...
            inst_regret_blocks.append(current_cf_action_values -
                                      current_cf_state_values)
        inst_regret_blocks.reverse()
        return tf.reshape(tf.concat(inst_regret_blocks, axis=0),
                          self.regrets.shape)
