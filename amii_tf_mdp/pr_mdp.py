...
# TODO This is probably still useful but needs to be changed to be usable.
def mean(mdp_params):
    z = float(len(mdp_params))
    mean_transition_model = mdp_params[0].transition_model / z
    mean_rewards = mdp_params[0].rewards / z
    for i in range(1, len(mdp_params)):
        mean_transition_model = (
            mean_transition_model + mdp_params[i].transition_model / z
        )
        mean_rewards = mean_rewards + mdp_params[i].rewards / z
...
def num_states(transitions): return transitions.shape[0].value
...
def pr_mdp_expected_value(
    horizon,
    num_states,
    num_actions,
    pr_mdp_sequence_probs,
    rewards,
    strat
):
    ...
    strat = tf.reshape(
        strat,
        [
            num_pr_prefixes(horizon - 1, num_states, num_actions),
            num_states,
            num_actions
        ]
    )
    ...
    for t in range(horizon - 1, -1, -1):
        ...
        current_cf_action_values = (
            current_rewards if current_cf_state_values is None
            else (
                current_rewards +
                tf.reshape(
